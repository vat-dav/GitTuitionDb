...
namespace TuitionDb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private TuitionDbUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<TuitionDbUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(TuitionDbUser)}'. " +
                    $"Ensure that '{nameof(TuitionDbUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<TuitionDbUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<TuitionDbUser>)_userStore;
        }
    }
}
